  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-96">
          <motion.div
              className="w-12 h-12 border-4 border-outline-variant border-t-primary-container rounded-full"
          animate={{ rotate: 360 }}
  ...
  if (!resume) {
    return (
      <div className="text-center py-20">
        ...
        <button onClick={onBack} className="px-6 py-2 bg-primary-container text-on-primary rounded-lg">返回列表</button>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex justify-between items-center mb-8 flex-wrap gap-4">
        <div className="flex items-center gap-4">
            ...
            className="w-10 h-10 bg-surface rounded-xl flex items-center justify-center text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface transition-all shadow-sm"
        ...
        <div className="flex gap-3">
          {detailView === 'interviewDetail' && selectedInterview && (
            <motion.button
              ...
              className="px-5 py-2.5 border border-outline-variant bg-surface rounded-xl text-on-surface-variant font-medium hover:bg-surface-container-high transition-all disabled:opacity-50 flex items-center gap-2"
          ...
          {detailView !== 'interviewDetail' && (
            <motion.button
              ...
              className="px-5 py-2.5 bg-primary-container text-on-primary rounded-xl font-medium shadow-lg shadow-sm hover:shadow-xl transition-all flex items-center gap-2"
      ...
      {/* 标签页切换 - 仅在非面试详情时显示 */}
      {detailView !== 'interviewDetail' && (
          <div className="bg-surface rounded-xl p-2 mb-6 inline-flex gap-1">
          {tabs.map((tab) => (
            <motion.button
              key={tab.id}
            ...
            >
              ...
              <span className="relative z-10 flex items-center gap-2">
                ...
                {tab.count !== undefined && tab.count > 0 && (
                    <span
                        className="px-2 py-0.5 bg-primary-container/10 text-primary-container text-xs rounded-full">{tab.count}</span>
                )}
              </span>
            </motion.button>
          ))}
